...
################################################################################
# Akuity For Managed ArgoCD
################################################################################

module "akuity" {
  source = "./modules/terraform-akuity-argocd"

  create = var.enable_akuity

  akuity_instance_id = try(var.akuity.instance_id, "")
  akuity_name        = try(var.akuity.name, "akuity-argocd")
  akuity_namespace   = try(var.akuity.namespace, "akuity")
  akuity_agent_size  = try(var.akuity.agent_size, "small")
...
################################################################################
# Google Artifact Registry Private Connection
################################################################################

data "aws_ssm_parameter" "gar_service_account_key" {
  count = var.enable_gar_registry_secret ? 1 : 0

  provider = aws.main
  name     = try(var.gar_registry.sa_key_ssm_name, "/eks/GAR_SERVICE_ACCOUNT_KEY")
}

resource "kubernetes_secret" "registry_secret_gar" {
  for_each = var.enable_gar_registry_secret ? toset(try(var.gar_registry.namespaces, ["default"])) : []

  metadata {
    name      = try(var.gar_registry.name, "gar-cfg")
    namespace = each.key
  }
  ...
  data = {
    ".dockerconfigjson" = jsonencode({
      auths = {
        "${try(var.gar_registry.server, "us-docker.pkg.dev")}" = {
          "email"    = var.gar_registry.email
          "username" = try(var.gar_registry.username, "_json_key")
          "password" = data.aws_ssm_parameter.gar_service_account_key[0].value
        }
      }
    })
  }
